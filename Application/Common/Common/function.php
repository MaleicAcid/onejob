<?php
//随机抽取数组,若抽取数量大于数组元素个数则数组翻倍
function arrayExtract($data=array(),$num){
    if ($num<=0 || sizeof($data)==$num) {
        return $data;
    }
    while (sizeof($data)<$num) {
...
{
    return strip_tags(htmlspecialchars_decode($str)); //输出 中文字符
}
/**
 * 友好的时间显示
 *
...
function getUserName($id){
    if (!is_numeric($id) && empty($id)) {
        ...
        $name = $comMod->where($id_where)->field('realname')->find();
...
function getCategoryPid($id){
    if (!is_numeric($id) && empty($id)) {
        ...
        $comMod = M('JobCategory');
        $id_where = array(
            'id' => $id
        );
        $name = $comMod->where($id_where)->field('pid')->find();
        return $name['pid'];
    }
}
...
function getCompanyState($id){
    if (!is_numeric($id) && empty($id)) {
        ...
        $name = $comMod->where($id_where)->field('state')->find();
        return $name['state'];
    }
}

function getCompanyIdByJid($jid){
    if (!is_numeric($jid) && empty($jid)) {
        return 0;
    }else {
        $comMod = M('Job');
        $id_where = array(
        ...
        $name = $comMod->where($id_where)->field('company_id')->find();
        return $name['company_id'];
    }
}

function getCompanyPid($id){
    if (!is_numeric($id) && empty($id)) {
        return 0;
    }else {
        $comMod = M('Company');
...
function getCompanyName($id){
    if (!is_numeric($id) && empty($id)) {
        ...
        $name = $comMod->where($id_where)->field('name')->find();
        return $name['name'];
    }
}

function getUserHeadPic($id){
    if (!is_numeric($id) && empty($id)) {
        return 0;
    }else {
        $comMod = M('User');
        ...
        $name = $comMod->where($id_where)->field('head_pic')->find();
        return get_https_img($name['head_pic']);
    }
}

function getCompanyHalfLogo($id){
    if (!is_numeric($id) && empty($id)) {
        return 0;
    }else {
        $comMod = M('Company');
        ...
        $name = $comMod->field('logo')->where($id_where)->find();
        return $name['logo'];
    }
...
function getCompanyLogo($id){
    if (!is_numeric($id) && empty($id)) {
        ...
        $name = $comMod->field('logo')->where($id_where)->find();
        return get_https_img($name['logo']);
    }

}
function get_https_img($path) {
    if(preg_match("/^(http:\/\/).*$/",$path)){
        return str_replace("http://","https://",$path);
    }else if (preg_match("/^(https:\/\/).*$/",$path)){
        return $path;
    }else{
        return C('HTTPS_SITE_URL').$path;
    }
}
...
function send_code_mess($phone,$key,$secret,$templateCode) {
    vendor('Alidayu.TopSdk'); //引入文件

    $c = new \TopClient;
    $c->appkey = $key;  //申请的appkey
    $c->secretKey = $secret;      //申请的secreKey
    $req = new \AlibabaAliqinFcSmsNumSendRequest;
    $req->setExtend("123456");
    $req->setSmsType("normal");
    $req->setSmsFreeSignName("就业直达号");  //测试名
// 生成验证码
    $code = rand(10000, 99999);
    $req->setSmsParam("{\"name\":\"用户\",\"cde\":\"$code\"}"); //回复信息
    $req->setRecNum($phone);//发送电话号码
    $req->setSmsTemplateCode($templateCode);//模板ID
