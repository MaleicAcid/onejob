...
<html lang="en">
	...
	<ul class="nav nav-list">
		...
		<li <?php if(link == 'notice'): ?>class="active open"<?php endif; ?>>
...
<div class="row-fluid">
	<h3 class="header blue lighter smaller">
		<div class="row-fluid  dataTables_wrapper">
			<div class="span4">
				<button class="btn btn-primary btn-small" onclick="$.lagou.formShow('<?php echo U('Admin/Link/add');?>');" type="button"><i class="icon-plus"></i>添加轮播图</button>
				<button class="btn btn-primary btn-small" onclick="$.updateLink('<?php echo U('Admin/Link/updateLink');?>');" type="button"><i class=""></i>推送更新到首页</button>
			</div>
		...
		<tbody>
			<?php foreach ($data as $key => $value): ?>
				<tr>
					...
					<td><a href="<?php echo (get_img($value["link_pic"])); ?>" target="_blank">点击查看</a></td>
					<td><?php echo ((isset($value["notice_id"]) && ($value["notice_id"] !== ""))?($value["notice_id"]):"无"); ?></td>
					<td><?php echo C('is_show')[$value['is_show']];?></td>
					<td><?php echo (date("Y-m-d H:i", $value["create_time"])); ?></td>
					<td>
						<div class="hidden-phone visible-desktop action-buttons">
							<a href="javascript:;" onclick="$.lagou.formShow('<?php echo U('Admin/Link/edit', array('id'=>$value['id']));?>');" class="blue tooltip-info no-hover-underline" data-rel="tooltip" data-original-title="编辑">
								<i class="icon-pencil bigger-130"></i>
							</a>
							<a href="javascript:;" onclick="$.del('<?php echo U('Admin/Link/delete', array('id'=>$value['id']));?>');" class="red tooltip-error no-hover-underline" data-rel="tooltip" data-original-title="删除图片"><i class="icon-trash bigger-130"></i></a>
						</div>
					</td>
				</tr>
			<?php endforeach ?>
